import {TestBed} from '@angular/core/testing';

import {MatSnackBarModule} from "@angular/material/snack-bar";
import {RouterTestingModule} from "@angular/router/testing";
import {RsaEncryptionService} from "./rsa-encryption.service";
import {HttpClientTestingModule} from "@angular/common/http/testing";
import {UserService} from "./user.service";
import {EncodingService} from "./encoding.service";
import {BrowserAnimationsModule} from "@angular/platform-browser/animations";
import {ErrorHandlingService} from "./error-handling.service";

describe('RsaEncryptionService', () => {
  let service: RsaEncryptionService;
  let encoding: EncodingService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [MatSnackBarModule, RouterTestingModule, HttpClientTestingModule, BrowserAnimationsModule],
      providers: [UserService, EncodingService, ErrorHandlingService]
    });
    service = TestBed.inject(RsaEncryptionService);
    encoding = TestBed.inject(EncodingService);
  });

  it('should be created', () => {
    expect(service).toBeTruthy();
  });

  it('test rsa encryption', async () => {
    localStorage.setItem('userInfo', '{"password":null,"encryptedPrivateKey":"-----BEGIN PRIVATE KEY-----\\nUnj2C8EdPKpPmEM\\n-----END PRIVATE KEY-----","publicKey":"-----BEGIN PUBLIC KEY-----\\\\n-----END PUBLIC KEY-----","enabled":false,"username":"user@email.com","authorities":[],"accountNonExpired":false,"accountNonLocked":false,"credentialsNonExpired":false}');
    service.decryptKeys('passphrase')
      .then(() => {
        const data = Array.from(crypto.getRandomValues(new Uint8Array(32)), (dec) => ('0' + dec.toString(16)).substr(-2)).join('');
          service.publicEncrypt(data)
            .then(encrypted => {
              service.privateDecrypt(encrypted)
                .then(decrypted => {
                  console.log(decrypted);
                  console.log(data);
                  expect(decrypted).toEqual(data);
                })
                .catch(err => console.log(err));
            })
      })

  });
});
